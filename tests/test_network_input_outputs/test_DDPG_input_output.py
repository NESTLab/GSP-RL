...
def test_building_critic_network():
    ...
    fc1_dims:int  = 200
    fc2_dims: int = 400
    DQN_Network = DDPGCriticNetwork(id, lr, input_size, action_size, fc1_dims, fc2_dims)
    for name, param in DQN_Network.named_parameters():
        shape = param.shape
        if name == 'fc1.weight':
...
def test_critic_forward():
    ...
    fc1_dims:int  = 200
    fc2_dims: int = 400
    DDPG_Critic_Network = DDPGCriticNetwork(id, lr, input_size, action_size, fc1_dims, fc2_dims)
