...
def test_building_critic_network():
    ...
    fc1_dims:int  = 200
    fc2_dims: int = 400
    DQN_Network = TD3CriticNetwork(id, beta, input_size, action_size, fc1_dims, fc2_dims)
    for name, param in DQN_Network.named_parameters():
        shape = param.shape
        if name == 'fc1.weight':
            assert(shape[1] == input_size+action_size)
            assert(shape[0] == fc1_dims)
        elif name == 'fc2.weight':
            assert(shape[1] == fc1_dims)
            assert(shape[0] == fc2_dims)
...
def test_critic_forward():
    ...
    fc1_dims:int  = 200
    fc2_dims: int = 400
    TD3_Critic_Network = TD3CriticNetwork(id, beta, input_size, action_size, fc1_dims, fc2_dims)
    random_input = T.rand((1, input_size))
    random_action = T.rand((1, action_size))
